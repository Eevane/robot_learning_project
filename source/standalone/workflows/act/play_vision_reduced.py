...
"""Run ACT without privileged object / target position inputs."""
...
parser = argparse.ArgumentParser(description="Play an ACT policy trained without object / target position inputs.")
...
def main() -> None:
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            action = torch.clamp(action.view(1, -1), min=-1.0, max=1.0)
            if args_cli.print_every > 0 and global_step % args_cli.print_every == 0:
                print(
                    f"[ACT-REDUCED] step={global_step} candidates={num_candidates} "
            ...
            if bool((terminated | truncated).item()):
                ...
                print(
                    f"[ACT-REDUCED] episode={episode_count} success={is_success} "
