...
"""Run a trained ACT-style policy on the Bi-ACT vision observation variant."""
...
parser = argparse.ArgumentParser(description="Play a trained ACT-style policy on the Bi-ACT vision task.")
parser.add_argument("--task", type=str, default="Isaac-Lift-Needle-PSM-IK-Abs-Bi-Vision-v0", help="Task name.")
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            if args_cli.print_every > 0 and global_step % args_cli.print_every == 0:
                print(
                    f"[ACT-BI] step={global_step} candidates={num_candidates} "
            ...
            if bool((terminated | truncated).item()):
                ...
                print(
                    f"[ACT-BI] episode={episode_count} success={is_success} "
